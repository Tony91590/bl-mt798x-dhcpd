...
detect_soc() {
    ...
    for dir in "$UBOOT_DIR/$CONFIGS_DIR_DEFAULT" \
        ...
        for file in "$dir"/*_"$BOARD"_defconfig \
                    "$dir"/*_"$BOARD"_multi_layout_defconfig; do
...
make -C "$ATF_DIR" -f "$ATF_MKFILE" "$ATF_CFG" \
    CONFIG_CROSS_COMPILER="$TOOLCHAIN" \
    CROSS_COMPILER="$TOOLCHAIN"
...
if [ -f "$ATF_DIR/build/${SOC}/release/fip.bin" ]; then
    ...
    cp -f "$ATF_DIR/build/${SOC}/release/fip.bin" \
        "output/${FIP_NAME}.bin"
...
if grep -Eq "(^_|CONFIG_TARGET_ALL_NO_SEC_BOOT=y)" "$ATF_CFG_PATH"; then
    ...
    if [ -f "$ATF_DIR/build/${SOC}/release/bl2.img" ]; then
        ...
        cp -f "$ATF_DIR/build/${SOC}/release/bl2.img" \
            "output/${BL2_NAME}.bin"

        echo "bl2-${SOC}_${BOARD}_${VERSION} build done"
        echo "Output: output/${BL2_NAME}.bin"
